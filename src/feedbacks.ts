	...
	self.setFeedbackDefinitions({
		...
		receive_message: {
			...
			callback: async (event, context): Promise<boolean> => {
				...
				let data: number[] = easymidi.parseMessage(String(opts.msgType), opts)
				if ((self.getFromDataStore(data) !== undefined) && (self.getFromDataStore(data) == self.getDataFromBytes(data).val)) {
					return true
				}
				return false
			},
